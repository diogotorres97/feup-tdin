...
module.exports = (sequelize, DataTypes) => {
  ...
  ReceiveStock.afterCreate(async (receiveStock) => {
    ...
    sendNotificationMessage(PUSHER_CHANNEL_STORE, messageType.createReceiveStock, {
      ...receiveStock.dataValues,
      Book: book,
    });
  });
  ...
  ReceiveStock.afterUpdate(async (receiveStock) => {
    const book = await receiveStock.getBook();
    sendNotificationMessage(PUSHER_CHANNEL_STORE, messageType.updateReceiveStock, {
      ...receiveStock.dataValues,
      Book: book,
    });
  });
