...
const getTopBooks = async () => {
  ...
  sells.forEach((element) => {
    ...
    if (topBooks[sell.bookId]) {
      topBooks[sell.bookId].total += parseInt(sell.total, 10);
    } else {
      topBooks[sell.bookId] = { total: parseInt(sell.total, 10), book: sell.Book };
    }
  });
  ...
  orders.forEach((element) => {
    ...
    if (topBooks[order.bookId]) {
      topBooks[order.bookId].total += parseInt(order.total, 10);
    } else {
      topBooks[order.bookId] = { total: parseInt(order.total, 10), book: order.Book };
    }
  });

  const keysSorted = Object.keys(topBooks).sort(
    (keyA, keyB) => topBooks[keyB].total - topBooks[keyA].total,
  );
